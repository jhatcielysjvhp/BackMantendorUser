...
READS SQL DATA
SQL SECURITY DEFINER
COMMENT 'Obtener todos los roles con paginación - SELECT directo (sin cursor)'
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        GET DIAGNOSTICS CONDITION 1
...
READS SQL DATA
SQL SECURITY DEFINER
COMMENT 'Obtener todos los usuarios con paginación y filtro - SELECT directo (sin cursor)'
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        GET DIAGNOSTICS CONDITION 1
    ...
    SET p_resultado = 0;
    SET p_mensaje = CONCAT('Se encontraron ');
END$$
...
READS SQL DATA
SQL SECURITY DEFINER
COMMENT 'Obtener todas las variables con paginación y filtro - SELECT directo (sin cursor)'
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        GET DIAGNOSTICS CONDITION 1
    ...
    SET p_resultado = 0;
    SET p_mensaje = CONCAT('Se encontraron variables');
END$$
